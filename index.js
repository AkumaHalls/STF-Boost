...
function scheduleReconnect(account) {
    ...
    const delay = Math.min(Math.pow(2, account.reconnect_attempts - 1) * 30000, 1800000); // 30s, 1m, 2m, 4m... max 30m
    const delayInMinutes = Math.round(delay / 60000);

    account.status = `Reconectando em ${delayInMinutes} min...`;
    console.log(`[${account.username}] Tentativa de reconexão #${account.reconnect_attempts}. Próxima tentativa em ${delayInMinutes} minutos.`);
