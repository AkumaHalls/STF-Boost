...
function setupListenersForAccount(account) {
    account.client.on('loggedOn', () => { console.log(`[${account.username}] Login OK!`); account.status = "Rodando"; account.sessionStartTime = Date.now(); account.reconnect_attempts = 0; applyLiveSettings(account); });
...
async function loadAccountsIntoMemory() {
    ...
    savedAccounts.forEach((acc, index) => {
        liveAccounts[acc.username] = { username: acc.username, password: decrypt(acc.password), games: acc.games || [730], settings: { ...defaultSettings, ...(acc.settings || {}) }, sentryFileHash: acc.sentryFileHash || null, status: 'Parado', client: new SteamUser(), sessionStartTime: null, steamGuardCallback: null, manual_logout: false, reconnect_attempts: 0 };
        setupListenersForAccount(liveAccounts[acc.username]);
        
        // LÓGICA DE INICIALIZAÇÃO ESCALONADA
        if (liveAccounts[acc.username].settings.autoRelogin) {
            const delay = index * 15000; // Atraso de 15 segundos por conta
            console.log(`[${acc.username}] Inicialização automática agendada em ${delay / 1000} segundos...`);
            setTimeout(() => {
                console.log(`[${acc.username}] A iniciar agora...`);
                liveAccounts[acc.username].status = "Iniciando...";
                // É importante que a UI seja atualizada para mostrar "Iniciando..."
                // mas a nossa UI já faz isso a cada poucos segundos, então está tudo bem.
                liveAccounts[acc.username].client.logOn(getLogonOptions(liveAccounts[acc.username]));
            }, delay);
        }
    });

    console.log(`${Object.keys(liveAccounts).length} contas carregadas e agendadas para iniciar.`);
}
