...
let plansCollection; 
let couponsCollection; // *** NOVA COLEÇÃO ***
let liveAccounts = {};
...
async function connectToDB() { 
    try { 
        ...
        couponsCollection = db.collection("coupons"); // *** NOVA ***
...
function startWorkerForAccount(accountData) {
    ...
    liveAccounts[username].startupTimeout = setTimeout(() => { if (liveAccounts[username]?.status === "Iniciando...") { sendDiscordNotification("❄️ Conta Congelada", "Timeout.", 16776960, username); worker.kill(); } }, 60000);
    ...
    worker.on('message', (msg) => {
        ...
        if (msg.type === 'statusUpdate') {
            if (liveAccounts[username].status !== msg.payload.status) { if (msg.payload.status === 'Rodando') sendDiscordNotification("✅ Conta Online", "Farmando.", 5763719, username); }
        ...
        if (!acc.manual_logout && acc.settings.autoRelogin) {
             usersCollection.findOne({ _id: new ObjectId(acc.ownerUserID) }).then(user => {
                if (!user || user.isBanned || (user.plan === 'free' && user.freeHoursRemaining <= 0) || (user.planExpiresAt && user.planExpiresAt < new Date())) {
                    ...
                    setTimeout(() => { if (liveAccounts[username]) { const decrypted = decrypt(acc.encryptedPassword); if (decrypted) startWorkerForAccount({ ...acc, password: decrypted }); } }, 30000);
                }
             });
...
async function loadAccountsIntoMemory() {
    ...
    savedAccounts.forEach((acc, index) => {
        ...
        setTimeout(() => { const pass = decrypt(acc.password); if (pass) startWorkerForAccount({ ...liveAccounts[acc.username], password: pass }); }, index * 15000);
    });
}
...
apiRouter.get('/auth-status', async (req, res) => {
    if (req.session.userId) {
        if (!req.session.username) { try { const user = await usersCollection.findOne({ _id: new ObjectId(req.session.userId) }); if (user) req.session.username = user.username; } catch(e) {} }
...
// *** ROTA ATUALIZADA: Checkout com Cupom ***
apiRouter.post('/create-checkout', isAuthenticated, async (req, res) => {
    ...
    // 2. Aplica Desconto (Validado no Servidor)
    if (couponCode) {
        ...
        if (coupon) {
            ...
            if (price < 0) price = 0;
            console.log(`[CHECKOUT] Desconto aplicado: ${coupon.discount}% (${coupon.code})`);
        }
    }
...
apiRouter.get('/status', isAuthenticated, (req, res) => { const accs = {}; for(const u in liveAccounts) { if(liveAccounts[u].ownerUserID === req.session.userId) { const a = liveAccounts[u]; accs[u] = { username: a.username, status: a.status, games: a.games, settings: a.settings, uptime: a.sessionStartTime ? Date.now() - a.sessionStartTime : 0 }; } } res.json({ accounts: accs }); });
...
apiRouter.post('/change-password', isAuthenticated, async (req, res) => { const h = await bcrypt.hash(req.body.newPassword, 10); await usersCollection.updateOne({ _id: new ObjectId(req.session.userId) }, { $set: { password: h } }); res.json({ message: "Alterada" }); });
// Bulk Actions
