...
const isAuthenticated = (req, res, next) => {
    ...
    // Adiciona o parâmetro de erro para a UI mostrar a mensagem
    res.redirect('/login?error=unauthorized');
};

// --- ESTRUTURA DE ROTAS CORRIGIDA ---

// 1. A Rota Principal (e única página protegida) é definida primeiro
app.get('/', isAuthenticated, (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'index.html'));
});

// 2. A API inteira é colocada sob um router protegido
const apiRouter = express.Router();
apiRouter.use(isAuthenticated); // Todas as rotas da API exigem login
...
app.use(apiRouter); // Usa o router para todas as rotas da API

// 3. Servir ficheiros estáticos vem por último
