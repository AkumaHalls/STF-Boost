...
function setupListenersForAccount(account) {
    account.client.on('loggedOn', () => { account.status = "Rodando"; account.sessionStartTime = Date.now(); account.reconnect_attempts = 0; applyLiveSettings(account); });
...
async function loadAccountsIntoMemory() {
    ...
    for (const acc of savedAccounts) {
        ...
        if (liveAccounts[acc.username].settings.autoRelogin) {
            console.log(`[${acc.username}] Inicialização automática ativada. A iniciar...`);
...
apiRouter.post('/start/:username', (req, res) => {
    ...
    // Se estiver Parado ou com Erro, inicia normalmente.
    // Se estiver "travado", reinicia o processo.
    if (status === 'Parado' || status === 'Erro' || status === 'Pendente: Steam Guard' || status.startsWith('Reconectando')) {
        console.log(`[${account.username}] Recebido comando de início/reinicio.`);
        account.status = "Iniciando...";
        account.reconnect_attempts = 0;
        account.manual_logout = false;
        
        // Se o cliente já estiver no meio de algo, desconecta primeiro para limpar o estado.
        if (account.client.steamID) {
            account.client.logOff();
        }
        // Inicia um novo login
        account.client.logOn(getLogonOptions(account));
        res.status(200).json({ message: "Processo iniciado/reiniciado." });
    } else {
        res.status(400).json({ message: "A conta já está rodando." });
    }
});
