...
// --- DEFINIÇÃO DOS ROUTERS (NO TOPO) ---
const apiRouter = express.Router();
const adminApiRouter = express.Router();

// --- CONSTANTES DE PLANOS E PREÇOS ---
...
// Limites Padrão (Fallback se o DB falhar)
let PLAN_LIMITS = {
...
async function connectToDB() { 
    try { 
        ...
        console.error("[DB] Erro fatal de conexão:", e); 
        process.exit(1); 
    } 
}
...
function startWorkerForAccount(accountData) {
    ...
    // Proteção contra crash do worker
...
app.use(session({ 
    secret: 'stfboost_secret_key_change_this', 
    resave: false, 
...
apiRouter.post('/create-checkout', async (req, res) => {
    ...
    if (planId === 'custom' && customConfig) {
        ...
        price = CUSTOM_PRICING.BASE + 
                (d * CUSTOM_PRICING.DAY) + 
                (a * CUSTOM_PRICING.ACCOUNT) + 
                (g * CUSTOM_PRICING.GAME);
...
apiRouter.get('/user-info', async (req, res) => {
    ...
    // *** LÓGICA DE LIMITES (Com suporte a Custom) ***
    const planDetails = user.customLimits || GLOBAL_PLANS[user.plan] || PLAN_LIMITS[user.plan] || PLAN_LIMITS['free'];
...
apiRouter.post('/add-account', async (req, res) => { 
    const { username, password } = req.body; const uid = req.session.userId; 
    ...
    // Limites
    const planDetails = user.customLimits || GLOBAL_PLANS[user.plan] || PLAN_LIMITS['free'];
    if (count >= planDetails.accounts) return res.status(403).json({ message: `Limite atingido.` }); 
...
apiRouter.post('/start/:username', async (req, res) => { 
    ...
    // Limites
    const planDetails = user.customLimits || GLOBAL_PLANS[user.plan] || PLAN_LIMITS['free'];
    if (acc.games.length > planDetails.games) return res.status(403).json({ message: "Limite jogos." }); 
...
apiRouter.post('/set-games/:username', async (req, res) => { 
    ...
    // Limites
    const planDetails = user.customLimits || GLOBAL_PLANS[user.plan] || PLAN_LIMITS['free'];
    if (games.length > planDetails.games) return res.status(403).json({ message: "Limite excedido." }); 
...
// ... (Resto das rotas normais: stop, remove, save, search, activate, my-keys, renew, change-pass, bulk...)
apiRouter.post('/stop/:username', (req, res) => { const acc = liveAccounts[req.params.username]; if (acc && acc.ownerUserID === req.session.userId) { acc.manual_logout = true; if (acc.worker) acc.worker.kill(); acc.status = "Parado"; res.json({ message: "OK" }); } });
...
apiRouter.post('/submit-guard/:username', (req, res) => { const acc = liveAccounts[req.params.username]; if (acc && acc.worker) { acc.worker.send({ command: 'submitGuard', data: { code: req.body.code } }); res.json({ message: "OK" }); } });
...
app.post('/api/mp-webhook', async (req, res) => {
    ...
    if (query.topic === 'payment' || query.type === 'payment') {
            ...
            if (payment.status === 'approved') {
                ...
                if (userId) {
                    ...
                    if (meta.plan_id === 'custom') {
                        ...
                        expiry.setDate(expiry.getDate() + parseInt(meta.custom_days));
                        
                        updateData.plan = 'custom';
                        updateData.planExpiresAt = expiry;
                        updateData.customLimits = {
                            accounts: parseInt(meta.custom_accounts),
                            games: parseInt(meta.custom_games)
                        };
                        console.log(`[MP] Plano CUSTOM ativado para ${userId}`);
                    } else {
        ...
        } catch (e) { console.error("[MP] Erro webhook:", e); }
    }
    res.sendStatus(200);
});
